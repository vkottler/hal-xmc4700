/**
 * \file
 * \brief Generated by ifgen (3.0.1).
 */
#pragma once

#include <cstdint>
#include <cstring>

namespace XMC4700
{

enum class USB0_EP1_DOEPTSIZ_ISO_RxDPID : uint8_t
{
    value1 /*!< DATA0 */,
    value2 = 1 /*!< DATA2 */,
    value3 = 2 /*!< DATA1 */,
    value4 = 3 /*!< MDATA */
};
static_assert(sizeof(USB0_EP1_DOEPTSIZ_ISO_RxDPID) == 1);

static constexpr uint16_t USB0_EP1_DOEPTSIZ_ISO_RxDPID_id = 3191;

/**
 * Converts USB0_EP1_DOEPTSIZ_ISO_RxDPID to a C string.
 *
 * \param[in] instance Value to convert.
 * \return             A C string representation of the value.
 */
inline const char *to_string(USB0_EP1_DOEPTSIZ_ISO_RxDPID instance)
{
    const char *result = "UNKNOWN USB0_EP1_DOEPTSIZ_ISO_RxDPID";

    switch (instance)
    {
    case USB0_EP1_DOEPTSIZ_ISO_RxDPID::value1:
        result = "value1";
        break;
    case USB0_EP1_DOEPTSIZ_ISO_RxDPID::value2:
        result = "value2";
        break;
    case USB0_EP1_DOEPTSIZ_ISO_RxDPID::value3:
        result = "value3";
        break;
    case USB0_EP1_DOEPTSIZ_ISO_RxDPID::value4:
        result = "value4";
        break;
    }

    return result;
}

/**
 * Converts a C string to USB0_EP1_DOEPTSIZ_ISO_RxDPID.
 *
 * \param[in]  data   A C string to convert.
 * \param[out] output The enumeration element to write.
 * \return            Whether or not the output was written.
 */
inline bool from_string(const char *data, USB0_EP1_DOEPTSIZ_ISO_RxDPID &output)
{
    bool result = false;

    if ((result = !strncmp(data, "value1", 6)))
    {
        output = USB0_EP1_DOEPTSIZ_ISO_RxDPID::value1;
    }
    else if ((result = !strncmp(data, "value2", 6)))
    {
        output = USB0_EP1_DOEPTSIZ_ISO_RxDPID::value2;
    }
    else if ((result = !strncmp(data, "value3", 6)))
    {
        output = USB0_EP1_DOEPTSIZ_ISO_RxDPID::value3;
    }
    else if ((result = !strncmp(data, "value4", 6)))
    {
        output = USB0_EP1_DOEPTSIZ_ISO_RxDPID::value4;
    }

    return result;
}

}; // namespace XMC4700
