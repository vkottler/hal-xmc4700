...
struct [[gnu::packed]] pref
{
     ...
     * Get PCON's DBYP bit.
     */
    inline PREF_PCON_DBYP get_PCON_DBYP()
    {
     ...
     * Get PCON's IBYP bit.
     */
    inline PREF_PCON_IBYP get_PCON_IBYP()
    {
     ...
     */
    inline void get_PCON(PREF_PCON_DBYP &DBYP, PREF_PCON_IBYP &IBYP)
    {
        uint32_t curr = PCON;

        DBYP = PREF_PCON_DBYP(curr & (1u << 4u));
        IBYP = PREF_PCON_IBYP(curr & (1u << 0u));
    }

    /**
     * Set all of PCON's bit fields.
     */
    inline void set_PCON(PREF_PCON_DBYP DBYP, PREF_PCON_IINV IINV, PREF_PCON_IBYP IBYP)
    {
        uint32_t curr = PCON;
