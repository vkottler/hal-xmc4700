/**
 * \file
 * \brief Generated by ifgen (3.1.1).
 */
#pragma once

#include "../enums/SDMMC_CON_SDMMC_CON_CDSEL.h"
#include "../enums/SDMMC_CON_SDMMC_CON_CDSVAL.h"
#include "../enums/SDMMC_CON_SDMMC_CON_WPSEL.h"
#include "../enums/SDMMC_CON_SDMMC_CON_WPSVAL.h"
#include "../ifgen/common.h"

namespace XMC4700
{

/**
 * SD and Multimediacard Control Register
 */
struct [[gnu::packed]] sdmmc_con
{
    /* Constant attributes. */
    static constexpr std::size_t size = 4; /*!< sdmmc_con's size in bytes. */

    /* Fields. */
    uint32_t SDMMC_CON; /*!< (read-write) SDMMC Configuration */

    /* Methods. */

    /**
     * Get SDMMC_CON's CDSVAL bit.
     */
    inline SDMMC_CON_SDMMC_CON_CDSVAL get_SDMMC_CON_CDSVAL() volatile
    {
        return SDMMC_CON_SDMMC_CON_CDSVAL(SDMMC_CON & (1u << 20u));
    }

    /**
     * Set SDMMC_CON's CDSVAL bit.
     */
    inline void set_SDMMC_CON_CDSVAL() volatile
    {
        SDMMC_CON |= 1u << 20u;
    }

    /**
     * Clear SDMMC_CON's CDSVAL bit.
     */
    inline void clear_SDMMC_CON_CDSVAL() volatile
    {
        SDMMC_CON &= ~(1u << 20u);
    }

    /**
     * Toggle SDMMC_CON's CDSVAL bit.
     */
    inline void toggle_SDMMC_CON_CDSVAL() volatile
    {
        SDMMC_CON ^= 1u << 20u;
    }

    /**
     * Get SDMMC_CON's CDSEL bit.
     */
    inline SDMMC_CON_SDMMC_CON_CDSEL get_SDMMC_CON_CDSEL() volatile
    {
        return SDMMC_CON_SDMMC_CON_CDSEL(SDMMC_CON & (1u << 16u));
    }

    /**
     * Set SDMMC_CON's CDSEL bit.
     */
    inline void set_SDMMC_CON_CDSEL() volatile
    {
        SDMMC_CON |= 1u << 16u;
    }

    /**
     * Clear SDMMC_CON's CDSEL bit.
     */
    inline void clear_SDMMC_CON_CDSEL() volatile
    {
        SDMMC_CON &= ~(1u << 16u);
    }

    /**
     * Toggle SDMMC_CON's CDSEL bit.
     */
    inline void toggle_SDMMC_CON_CDSEL() volatile
    {
        SDMMC_CON ^= 1u << 16u;
    }

    /**
     * Get SDMMC_CON's WPSVAL bit.
     */
    inline SDMMC_CON_SDMMC_CON_WPSVAL get_SDMMC_CON_WPSVAL() volatile
    {
        return SDMMC_CON_SDMMC_CON_WPSVAL(SDMMC_CON & (1u << 4u));
    }

    /**
     * Set SDMMC_CON's WPSVAL bit.
     */
    inline void set_SDMMC_CON_WPSVAL() volatile
    {
        SDMMC_CON |= 1u << 4u;
    }

    /**
     * Clear SDMMC_CON's WPSVAL bit.
     */
    inline void clear_SDMMC_CON_WPSVAL() volatile
    {
        SDMMC_CON &= ~(1u << 4u);
    }

    /**
     * Toggle SDMMC_CON's WPSVAL bit.
     */
    inline void toggle_SDMMC_CON_WPSVAL() volatile
    {
        SDMMC_CON ^= 1u << 4u;
    }

    /**
     * Get SDMMC_CON's WPSEL bit.
     */
    inline SDMMC_CON_SDMMC_CON_WPSEL get_SDMMC_CON_WPSEL() volatile
    {
        return SDMMC_CON_SDMMC_CON_WPSEL(SDMMC_CON & (1u << 0u));
    }

    /**
     * Set SDMMC_CON's WPSEL bit.
     */
    inline void set_SDMMC_CON_WPSEL() volatile
    {
        SDMMC_CON |= 1u << 0u;
    }

    /**
     * Clear SDMMC_CON's WPSEL bit.
     */
    inline void clear_SDMMC_CON_WPSEL() volatile
    {
        SDMMC_CON &= ~(1u << 0u);
    }

    /**
     * Toggle SDMMC_CON's WPSEL bit.
     */
    inline void toggle_SDMMC_CON_WPSEL() volatile
    {
        SDMMC_CON ^= 1u << 0u;
    }

    /**
     * Get all of SDMMC_CON's bit fields.
     */
    inline void get_SDMMC_CON(SDMMC_CON_SDMMC_CON_CDSVAL &CDSVAL, SDMMC_CON_SDMMC_CON_CDSEL &CDSEL, SDMMC_CON_SDMMC_CON_WPSVAL &WPSVAL, SDMMC_CON_SDMMC_CON_WPSEL &WPSEL) volatile
    {
        uint32_t curr = SDMMC_CON;

        CDSVAL = SDMMC_CON_SDMMC_CON_CDSVAL(curr & (1u << 20u));
        CDSEL = SDMMC_CON_SDMMC_CON_CDSEL(curr & (1u << 16u));
        WPSVAL = SDMMC_CON_SDMMC_CON_WPSVAL(curr & (1u << 4u));
        WPSEL = SDMMC_CON_SDMMC_CON_WPSEL(curr & (1u << 0u));
    }

    /**
     * Set all of SDMMC_CON's bit fields.
     */
    inline void set_SDMMC_CON(SDMMC_CON_SDMMC_CON_CDSVAL CDSVAL, SDMMC_CON_SDMMC_CON_CDSEL CDSEL, SDMMC_CON_SDMMC_CON_WPSVAL WPSVAL, SDMMC_CON_SDMMC_CON_WPSEL WPSEL) volatile
    {
        uint32_t curr = SDMMC_CON;

        curr &= ~(0b1u << 20u);
        curr |= (std::to_underlying(CDSVAL) & 0b1u) << 20u;
        curr &= ~(0b1u << 16u);
        curr |= (std::to_underlying(CDSEL) & 0b1u) << 16u;
        curr &= ~(0b1u << 4u);
        curr |= (std::to_underlying(WPSVAL) & 0b1u) << 4u;
        curr &= ~(0b1u << 0u);
        curr |= (std::to_underlying(WPSEL) & 0b1u) << 0u;

        SDMMC_CON = curr;
    }
};

static_assert(sizeof(sdmmc_con) == sdmmc_con::size);

static volatile sdmmc_con *const SDMMC_CON = reinterpret_cast<sdmmc_con *>(0x500040B4);

}; // namespace XMC4700
