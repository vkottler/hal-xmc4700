...
struct [[gnu::packed]] gpdma0_ch2
{
     ...
     * Channel Suspend
     */
    inline bool get_CFGL_CH_SUSP() volatile
    {
        return CFGL & (1u << 8u);
    }
     ...
     */
    inline bool get_CFGL_FIFO_EMPTY() volatile
    {
        return CFGL & (1u << 9u);
    }
     ...
     */
    inline bool get_CFGL_HS_SEL_DST() volatile
    {
        return CFGL & (1u << 10u);
    }
     ...
     */
    inline bool get_CFGL_HS_SEL_SRC() volatile
    {
        return CFGL & (1u << 11u);
    }
     ...
     */
    inline bool get_CFGL_DST_HS_POL() volatile
    {
        return CFGL & (1u << 18u);
    }
     ...
     */
    inline bool get_CFGL_SRC_HS_POL() volatile
    {
        return CFGL & (1u << 19u);
    }
     ...
     */
    inline void get_CFGL(uint8_t &CH_PRIOR, bool &CH_SUSP, bool &FIFO_EMPTY,
                         bool &HS_SEL_DST, bool &HS_SEL_SRC,
                         GPDMA0_CH2_CFGL_LOCK_CH_L &LOCK_CH_L,
                         GPDMA0_CH2_CFGL_LOCK_CH_L &LOCK_B_L, bool &LOCK_CH,
                         bool &LOCK_B, bool &DST_HS_POL, bool &SRC_HS_POL,
                         uint16_t &MAX_ABRST) volatile
    {
        ...
        CH_SUSP = curr & (1u << 8u);
        FIFO_EMPTY = curr & (1u << 9u);
        HS_SEL_DST = curr & (1u << 10u);
        HS_SEL_SRC = curr & (1u << 11u);
        LOCK_CH_L = GPDMA0_CH2_CFGL_LOCK_CH_L((curr >> 12u) & 0b11u);
        LOCK_B_L = GPDMA0_CH2_CFGL_LOCK_CH_L((curr >> 14u) & 0b11u);
        LOCK_CH = curr & (1u << 16u);
        LOCK_B = curr & (1u << 17u);
        DST_HS_POL = curr & (1u << 18u);
        SRC_HS_POL = curr & (1u << 19u);
     ...
     * (read-write) Configuration Register Low
     */
    inline void set_CFGL(uint8_t CH_PRIOR, bool CH_SUSP, bool HS_SEL_DST,
                         bool HS_SEL_SRC, GPDMA0_CH2_CFGL_LOCK_CH_L LOCK_CH_L,
                         GPDMA0_CH2_CFGL_LOCK_CH_L LOCK_B_L, bool LOCK_CH,
                         bool LOCK_B, bool DST_HS_POL, bool SRC_HS_POL,
                         uint16_t MAX_ABRST) volatile
    {
        ...
        curr &= ~(0b1u << 8u);
        curr |= (CH_SUSP & 0b1u) << 8u;
        curr &= ~(0b1u << 10u);
        curr |= (HS_SEL_DST & 0b1u) << 10u;
        curr &= ~(0b1u << 11u);
        curr |= (HS_SEL_SRC & 0b1u) << 11u;
        curr &= ~(0b11u << 12u);
        ...
        curr &= ~(0b1u << 18u);
        curr |= (DST_HS_POL & 0b1u) << 18u;
        curr &= ~(0b1u << 19u);
        curr |= (SRC_HS_POL & 0b1u) << 19u;
        curr &= ~(0b1111111111u << 20u);
     ...
     * Flow Control Mode
     */
    inline bool get_CFGH_FCMODE() volatile
    {
        return CFGH & (1u << 0u);
    }
     ...
     * FIFO Mode Select
     */
    inline bool get_CFGH_FIFO_MODE() volatile
    {
        return CFGH & (1u << 1u);
    }
     ...
     */
    inline void get_CFGH(bool &FCMODE, bool &FIFO_MODE, uint8_t &PROTCTL,
                         uint8_t &SRC_PER, uint8_t &DEST_PER) volatile
    {
        uint32_t curr = CFGH;

        FCMODE = curr & (1u << 0u);
        FIFO_MODE = curr & (1u << 1u);
     ...
     * (read-write) Configuration Register High
     */
    inline void set_CFGH(bool FCMODE, bool FIFO_MODE, uint8_t PROTCTL,
                         uint8_t SRC_PER, uint8_t DEST_PER) volatile
    {
        uint32_t curr = CFGH;
